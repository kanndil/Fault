    ...
    struct ATPG: ParsableCommand {
        ...
        mutating func run() throws {
            ...
            do {
                ...
                if etvSetVectors.count > 0 {
                    var etvInputsMinusIgnored: [Port] = []
                    var offset = 0
                    for (i, input) in etvSetInputs.enumerated() {
                        if bypass.bypassedInputs.contains(input.name) {
                            ...
                            etvInputsMinusIgnored.append(input)
                        }
                    }
                    let netlistPortNames = Set<String>(inputsMinusIgnored.map({ $0.name }))
                    let benchPortNames = Set<String>(etvInputsMinusIgnored.map({ $0.name }))
                    if netlistPortNames.count != benchPortNames.count {
                        let all = netlistPortNames.union(benchPortNames)
                        let onlyInBench = all.subtracting(netlistPortNames)
                        let onlyInNetlist = all.subtracting(benchPortNames)
                        if onlyInBench.count > 0 {
                            print(
                                "Extraneous ports found only in external test vectors:"
                            )
                            for port in onlyInBench.sorted() {
                                print("\t*\(port)")
                            }
                        }
                        if onlyInNetlist.count > 0 {
                            print(
                                "Ports missing from external test vectors:"
                            )
                            for port in onlyInNetlist.sorted() {
                                print("\t*\(port)")
                            }
                    ...
                    inputsMinusIgnored = etvInputsMinusIgnored
                }
